...
const TrendingLinkModal = ({ isOpen, onClose, item, refresh, editMode }) => {
  ...
  const updateDesign = async () => {
    ...
    try {
      ...
      let response
      if (editMode) {
        response = await axios.patch('/api/admin/designs/trending-link', {
          id: item?.id,
          link: newHeroSlide.link,
        })
      } else {
        response = await axios.post('/api/admin/designs/trending-link', {
          link: newHeroSlide.link,
        })
      }
  ...
  return (
    <Modal size={'sm'} isOpen={isOpen}>
      <ModalContent>
        <ModalHeader>
          {' '}
          {editMode ? 'Edit Trending Link' : 'Add Trending Link'}
...
export default TrendingLinkModal
