...
import TrendingLinkModal from './TrendingLinkModal'

const TrendingLinkData = () => {
  ...
  const fetchLinks = async () => {
    try {
      const response = await axios.get('/api/admin/designs/trending-link')
  ...
  const deleteData = async () => {
    try {
      await axios.delete('/api/admin/designs/trending-link', {
    ...
    <div>
      <div className='bg-white p-3'>
        <div className='flex justify-between px-[1px]'>
          <p className='text-base font-bold'>Trending Link</p>
          <Button
            label={'Add Link'}
            onClick={() => {
              setEditMode(false)
...
export default TrendingLinkData
